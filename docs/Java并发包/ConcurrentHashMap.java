...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     * 按key和cv匹配这个结点，把这个结点的值替换为value,如果value等于null,则删除这个结点
     */
                            ...
                            for (Node<K,V> e = f, pred = null;;) {
                                ...
                                if (e.hash == hash &&
                                    ...
                                    if (cv == null || cv == ev ||
                                        ...
                                        if (value != null)
                                            e.val = value;
                                        // value=null，pred是e的前驱结点，pred!=null直接删除e结点
                                        else if (pred != null)
                                            pred.next = e.next;
                                        // value=null，pred=null表示e没有前驱结点，此时e就是首结点，cas设置tab[i]=e.next，删除e结点
                                        else
    ...
    public void clear() {
        ...
        while (tab != null && i < tab.length) {
            ...
            if (f == null)
                ++i;
            // 正在扩容，就协作扩容
            // 这里为什么要协作扩容？因为不去协作扩容，就没有办法删除，扩容加锁了，这里获取不到锁，干等着
            else if ((fh = f.hash) == MOVED) {
                tab = helpTransfer(tab, f);
                // 扩容完，再把i设置0,从头删除
                i = 0; // restart
     ...
     * numberOfLeadingZeros返回无符号整型i的最高非零位前面的0的个数,
     * Integer.numberOfLeadingZeros(n)和(1 << (RESIZE_STAMP_BITS - 1)做或运算，相当于将
     * 两个数相加
     */
